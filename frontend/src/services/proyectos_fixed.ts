...
export const proyectosService = {
  getAll: async (): Promise<Proyecto[]> => {
    try {
      const response = await api.get('/api/proyectos/proyectos/');
  ...
  getById: async (id: number): Promise<Proyecto> => {
    try {
      const response = await api.get<Proyecto>(`/api/proyectos/proyectos/${id}/`);
  ...
  create: async (proyecto: Omit<Proyecto, 'id'>): Promise<Proyecto> => {
    try {
      ...
      const response = await api.post<Proyecto>('/api/proyectos/proyectos/', formData, {
        headers: {
  ...
  update: async (id: number, proyecto: Partial<Proyecto>): Promise<Proyecto> => {
    try {
      ...
      const response = await api.patch<Proyecto>(`/api/proyectos/proyectos/${id}/`, formData, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
      });
  ...
  delete: async (id: number): Promise<void> => {
    try {
      await api.delete(`/api/proyectos/proyectos/${id}/`);
    } catch (error) {
